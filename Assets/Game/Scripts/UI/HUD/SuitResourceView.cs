...
namespace PlanetSurvival.UI.HUD
{
    ...
    public sealed class SuitResourceView : MonoBehaviour
    {
        ...
        private SpaceSuitResources _resources;
        ...
        public void Bind(SpaceSuitResources resources)
        {
            ...
            Debug.LogError($"{nameof(SuitResourceView)} on '{name}' requires suit resources.", this);
            enabled = false;
        }
        ...
        private void OnGUI()
        {
            ...
            EnsureStyles();
            var panel = new Rect(Margin, Margin, PanelWidth, PanelHeight);
            PanelBackground.Draw(panel);
            GUI.Label(new Rect(panel.x + 14f, panel.y + 8f, panel.width - 28f, 22f),
                "SPACE SUIT RESOURCES", _headerStyle);

            DrawResource(panel.y + 36f, _oxygenTexture,
                _resources.Oxygen.CurrentLiters,
                _resources.Oxygen.CapacityLiters,
                "L",
                new Color(.35f, .85f, 1f));
            DrawResource(panel.y + 78f, _waterTexture,
                _resources.Water.CurrentMilliliters / 1000f,
                _resources.Water.CapacityMilliliters / 1000f,
                "L",
                new Color(.3f, .62f, 1f));
        }
        ...
        private void DrawResource(float y, Texture2D icon, float current, float capacity, string unit, Color color)
        {
            ...
            if (icon != null)
            {
                GUI.DrawTexture(new Rect(Margin + padding, y, iconSize, iconSize), icon,
                    ScaleMode.ScaleToFit, true);
            }

            var valueRect = new Rect(Margin + padding + iconSize + 8f, y, PanelWidth - padding * 2f - iconSize - 8f, 18f);
